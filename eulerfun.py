...
def factorial(n):
    return functools.reduce(op.mul, range(1, n + 1))
...
def pal_list(k):
    ...
    return [sum([n * (10 ** i) for i, n in enumerate(
        ([x] + list(ys) + [z] + list(ys)[::-1] + [x]) if k % 2 else ([x] + list(ys) + list(ys)[::-1] + [x]))])
            for x in range(1, 10)
...
http://en.literateprograms.org/Miller-Rabin_primality_test_(Python)
    ...
    for repeat in range(20):
        ...
        if not miller_rabin_pass(a, s, d2, n):
            return False
    return True


def miller_rabin_pass(a, s, d3, n):
    a_to_power = pow(a, d3, n)
    if a_to_power == 1:
        return True
    for i in range(s - 1):
        if a_to_power == n - 1:
