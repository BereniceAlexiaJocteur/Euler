...
class Problem():
    ...
    def __init__(self):
        ...
        self.n = 20000
        self.mod = 10**9 + 7
        self.curr_factorisation = dict()
        self.curr_sum_factors = None
        self.curr_factoriel_factorisation = dict()
        self.dico_inverse_primes_minus_1 = dict()
        self.list_factorisation_initiale = []  # en 0 factorisation de 2 en 1 celle de 3 etc
        self.list_factorisation_initiale_oposee = []
    ...
    def init_list_factorisations(self):
        ...
        self.list_factorisation_initiale_oposee = copy.deepcopy(self.list_factorisation_initiale)
        for i in range(len(self.list_factorisation_initiale_oposee)):
            self.list_factorisation_initiale_oposee[i] = self.mult_dico(self.list_factorisation_initiale_oposee[i], -1)

    def get_res(self):
        for i in range(2, self.n+1):
            self.curr_factoriel_factorisation = self.add_two_dicos(self.curr_factoriel_factorisation, self.list_factorisation_initiale_oposee[i-2])
            # calcul B(i)
            self.curr_factorisation = self.add_two_dicos(self.curr_factorisation, self.mult_dico(self.list_factorisation_initiale[i-2], i))
            ...
            for j in self.curr_factorisation:
                ...
                self.curr_sum_factors = (self.curr_sum_factors*((pow(j, self.curr_factorisation[j]+1, self.mod)-1)*self.dico_inverse_primes_minus_1[j])) % self.mod
